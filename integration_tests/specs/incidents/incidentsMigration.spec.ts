...
test.describe('Incident Migration Homepage', () => {
  ...
  test.describe('Without MIGRATE_NOMIS_SYSCON role', () => {
    ...
    test('should not see migrate incident tile', async ({ page }) => {
      const indexPage = await IndexPage.verifyOnPage(page)
      await expect(indexPage.migrationLink(migrationTypeName)).toBeHidden()
    })

    test('should not be able to navigate directly to the incidents migration page', async ({ page }) => {
      await page.goto('/incidents-migration')
