...
import VisitsMigrationPage from '../../pages/visits/visitsMigrationPage'

const migrationType: string = 'VISITS'
const migrationTypeName: string = 'Visits'

test.describe('Visit Migration Homepage', () => {
  ...
  test.describe('With MIGRATE_VISITS role', () => {
    ...
    test('should see migrate visits tile', async ({ page }) => {
      const indexPage = await IndexPage.verifyOnPage(page)
      await expect(indexPage.migrationLink(migrationTypeName)).toBeVisible()
    })

    test('should be able to navigate to the visits migration home page', async ({ page }) => {
      const indexPage = await IndexPage.verifyOnPage(page)
    ...
    test('should display list of migrations', async ({ page }) => {
      ...
      await expect(row0.getByTestId('filterPrisonIds')).toHaveText('HEI')
      await expect(row0.getByTestId('filterVisitTypes')).toHaveText('SCON')
      await expect(row0.getByTestId('filterToDate')).toBeHidden()
      await expect(row0.getByTestId('filterFromDate')).toHaveText('4 March 2022 - 16:01')
      ...
      await expect(row1.getByTestId('filterPrisonIds')).toHaveText('HEI')
      await expect(row1.getByTestId('filterVisitTypes')).toHaveText('SCON')
      ...
      await expect(row2.getByTestId('filterPrisonIds')).toHaveText('MDI,HEI')
      await expect(row2.getByTestId('filterVisitTypes')).toHaveText('SCON')
      await expect(row2.getByTestId('filterToDate]')).toBeHidden()
      await expect(row2.getByTestId('filterFromDate')).toHaveText('15 March 2022 - 09:01')
    ...
    test('will validate the dates when filtering', async ({ page }) => {
      const indexPage = await IndexPage.verifyOnPage(page)
      await indexPage.migrationLink(migrationTypeName).click()
      const migrationPage = await VisitsMigrationPage.verifyOnPage(migrationTypeName, page)

      await migrationPage.fromDateTime.fill('invalid')
      await migrationPage.toDateTime.fill('1971-67-12')
      await migrationPage.applyFiltersButton.click()

      const pageWithErrors = await VisitsMigrationPage.verifyOnPage(migrationTypeName, page)
      await expect(pageWithErrors.fromDateTimeError).toHaveText(
        'Error: Enter a real date time, like 2020-03-23T12:00:00 or 2020-03-23',
      )
      await expect(pageWithErrors.toDateTimeError).toHaveText(
        'Error: Enter a real date time, like 2020-03-23T12:00:00 or 2020-03-23',
      )
    })
  })
  ...
  test.describe('Without MIGRATE_VISITS role', () => {
    ...
    test('should not see migrate visits tile', async ({ page }) => {
      const indexPage = await IndexPage.verifyOnPage(page)
      await expect(indexPage.migrationLink(migrationTypeName)).toBeHidden()
    })
    test('should not be able to navigate directly to the visits migration page', async ({ page }) => {
      await page.goto('/visits-migration')
