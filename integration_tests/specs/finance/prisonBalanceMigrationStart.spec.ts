...
test.describe('Prison Balance Migration Start', () => {
  ...
  test.describe('With MIGRATE_NOMIS_SYSCON role', () => {
    ...
    test('will validate page when selecting start migration', async ({ page }) => {
      ...
      await startMigrationPage.getField('prisonId').fill('invalid')
    ...
    test('will ensure prison id is optional when selecting start migration', async ({ page }) => {
      ...
      await nomisPrisonerApi.stubGetPrisonBalanceMigrationEstimatedCount(100_988)
      ...
      await expect(previewPage.estimateSummary).toHaveText(
        'Estimated number of prison balance entities to be migrated: 100,988',
      )
    })
    ...
    test('Preview of migration will be shown and changes allowed prior to starting a migration', async ({ page }) => {
      ...
      await nomisPrisonerApi.stubGetPrisonBalanceMigrationEstimatedCount(1)
      ...
      const previewPage = await StartMigrationPreviewPage.verifyOnPage(migrationTypeName, page)
      await expect(previewPage.estimateSummary).toHaveText(
        'Estimated number of prison balance entities to be migrated: 1',
      )
      ...
      await startMigration2Page.getField('prisonId').fill('DNI')
      ...
      await expect(previewPageAgain.getFieldRow('prison-id')).toContainText('DNI')
