...
import staffMigrationHistory from '../../mockApis/nomisStaffMigrationApi'

const migrationType: string = 'STAFF'
const migrationTypeName: string = 'Staff'

test.describe('Staff Migration Homepage', () => {
  test.afterEach(async () => {
    await resetStubs()
  })
  test.describe('With MIGRATE_NOMIS_SYSCON role', () => {
    test.beforeEach(async ({ page }) => {
      await nomisMigrationApi.stubGetMigrationHistory({ migrationType, history: staffMigrationHistory })
      await login(page)
    })

    test('should see migrate staff tile', async ({ page }) => {
      const indexPage = await IndexPage.verifyOnPage(page)
      await expect(indexPage.migrationLink(migrationTypeName)).toBeVisible()
    })

    test('should be able to navigate to the staff migration home page', async ({ page }) => {
      const indexPage = await IndexPage.verifyOnPage(page)
  ...
  test.describe('Without MIGRATE_NOMIS_SYSCON role', () => {
    ...
    test('should not be able to navigate directly to the staff migration page', async ({ page }) => {
      await page.goto('/staff-migration')
