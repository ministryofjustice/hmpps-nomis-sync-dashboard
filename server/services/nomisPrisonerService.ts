...
import { GetVisitsByFilter, PageVisitIdResponse, RoomMappingsResponse } from '../@types/nomisPrisoner'
...
export default class NomisPrisonerService {
  ...
  async getVisitMigrationRoomMappings(filter: GetVisitsByFilter, context: Context): Promise<RoomMappingsResponse[]> {
    ...
    return NomisPrisonerService.restClient(token).get<RoomMappingsResponse[]>({
      path: `/visits/rooms/usage-count`,
      query: `${querystring.stringify({ ...filter, size: 1 })}`,
    })
  }
}
