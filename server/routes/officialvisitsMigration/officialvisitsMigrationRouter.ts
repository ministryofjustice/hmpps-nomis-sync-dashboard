...
import OfficialvisitsMigrationController from './officialvisitsMigrationController'
import NomisMigrationService from '../../services/nomisMigrationService'
import authorisationMiddleware from '../../middleware/authorisationMiddleware'
import { MIGRATE_NOMIS_SYSCON } from '../../authentication/roles'

export default function routes({
  officialvisitsNomisMigrationService,
  officialvisitsNomisPrisonerService,
  nomisMigrationService,
}: {
  officialvisitsNomisMigrationService: OfficialvisitsNomisMigrationService
  officialvisitsNomisPrisonerService: OfficialvisitsNomisPrisonerService
  nomisMigrationService: NomisMigrationService
}): Router {
  const router = express.Router({ mergeParams: true })
  router.use(authorisationMiddleware([MIGRATE_NOMIS_SYSCON]))

  const migrationController = new OfficialvisitsMigrationController(
    officialvisitsNomisMigrationService,
    officialvisitsNomisPrisonerService,
    nomisMigrationService,
  )
