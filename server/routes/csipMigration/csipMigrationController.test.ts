...
describe('CSIPMigrationController', () => {
  const req = {
    query: {},
    session: {},
    flash: jest.fn(),
  ...
  describe('getCSIPMigrations', () => {
    it('should decorate the returned migrations', async () => {
      const csipMigrationResponse: HistoricMigrations = {
        migrations: [
          {
      ...
      nomisMigrationService.getMigrationHistory.mockResolvedValue(csipMigrationResponse)

      await new CSIPMigrationController(nomisMigrationService, nomisPrisonerService).getCSIPMigrations(req, res)
      expect(res.render).toBeCalled()
      expect(res.render).toBeCalledWith('pages/csip/csipMigration', {
  ...
  describe('viewFailures', () => {
    beforeEach(() => {
      nomisMigrationService.getCSIPFailures.mockResolvedValue({
    ...
    it('should render the failures page with application insights link for failed messageId', async () => {
      await new CSIPMigrationController(nomisMigrationService, nomisPrisonerService).viewFailures(req, res)
      expect(res.render).toBeCalledWith('pages/csip/csipMigrationFailures', {
  ...
  describe('postStartCSIPMigration', () => {
    describe('with validation error', () => {
      it('should return an error response', async () => {
        ...
        await new CSIPMigrationController(nomisMigrationService, nomisPrisonerService).postStartCSIPMigration(req, res)
        expect(req.flash).toBeCalledWith('errors', [{ href: '#toDate', text: 'Enter a real date, like 2020-03-23' }])
        expect(res.redirect).toHaveBeenCalledWith('/csip-migration/amend')
      })
    })
  })
})
